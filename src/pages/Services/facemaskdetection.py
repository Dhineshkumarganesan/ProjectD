...
path ="src/pages/Services/face_detector/"
model_path="src/pages/Services/models/"
image_folder='src/pages/Services/images/'
...
def get_file_name(file: Union[BytesIO, StringIO], folder_path=image_folder):
...
def load_face_detector_and_model():
    prototxt_path = os.path.sep.join([path, "deploy.prototxt"])
    weights_path = os.path.sep.join([path,"res10_300x300_ssd_iter_140000.caffemodel"])
...
def load_cnn_model():
    cnn_model = load_model(model_path+"mask_detector.model")
