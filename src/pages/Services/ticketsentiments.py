...
def write():
	...
	#path ="C:\\ProjD\\25FinD\\src\\pages\\Services\\models\\"
	...
	if choice == 'Prediction':
		...
		if st.button("Classify"):
			...
			if model_choice == 'LR':
				predictor = load_prediction_models(path +"newsclassifier_Logit_model.pkl")
				prediction = predictor.predict(vect_text)
				# st.write(prediction)
			elif model_choice == 'RFOREST':
				predictor = load_prediction_models(path +"newsclassifier_RFOREST_model.pkl")
				prediction = predictor.predict(vect_text)
				# st.write(prediction)
			#elif model_choice == 'NB':
				#predictor = load_prediction_models(path +"newsclassifier_NB_model.pkl")
				#prediction = predictor.predict(vect_text)
				# st.write(prediction)
			elif model_choice == 'DECISION_TREE':
				predictor = load_prediction_models(path +"newsclassifier_CART_model.pkl")
				prediction = predictor.predict(vect_text)
				# st.write(prediction)
